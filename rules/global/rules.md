...
`.claude/injectable/customization.md` is the only one that stands apart: no `common/` subject talks about it, because it gathers preferences that cut across all the work on the project, like the language the conversation is held in. A single list, under "Global rules to follow": one rule per line, one default line saying that none is imposed until the project has added one.
