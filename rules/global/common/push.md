...
**A project can write the message differently.** When `.claude/injectable/push.md` exists, it says what this project does instead of the tags, the format, the language, or the message footer given further down. The attribution ban below never falls under what gets customized, whatever `.claude/injectable/push.md` writes. The rest, splitting into scopes and how to find it, doesn't get customized either and stays defined here.
